...
function travel() {
    ...
    while (locations.chooseLocation === true) {
        choice = prompt(`Choose a number: `);
        if (choice === `1`) {
            locations.bar = true;
            locations.chooseLocation = false
        } else if (choice === `2`) {
            locations.home = true;
            locations.chooseLocation = false
        } else if (choice === `3`) {
...
function checkGameOver() {
    if (chara.health <= 0) {
        ...
        console.log("\nThis is part of an going project. Stay alive and stay tuned for more coming soon...");
        process.exit();
...
// TO ADD - FACTION ACTIONS LIST.

// location branches


while (chooseLocation === true) {
    showStats();
    travel();
        ...
        while (locations.hideOut === true) {
            if (chara.inventory === 'keycard') {
                ...
                chara.health -= 10
                chara.sanity -= 5
                console.log(`You lose 10 health and 5 sanity. Here's how much you have left:\n |HEALTH| ${chara.health}\n |SANITY| ${chara.sanity} `);
                checkGameOver();
